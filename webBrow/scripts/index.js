(function() {
  ...
  Calculator.prototype.addTheListeners = function() {
    ...
    currentCalc.addEventListener('click', function(event){
      if(event.target.matches('.calculator-button:not(flip):not(percentage), decimal')){
        calculator.displayValue(event.target.innerText)
      }else if(event.target.matches('divide, multiply, subtract, add')){
        calculator.getOperand(event.target.innerText)
  ...
  Calculator.prototype.identifyOperand = function(data){
    console.log('hererere');
    var someHeaders = new Headers()
    someHeaders.append('Content-type', 'application/json')
    someHeaders.append('Success', 'application/json')
